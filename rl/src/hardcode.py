...
class Scout():
    def __init__(self,size = 16,is_scout= True,first=False,local = True,map_generator=None):   
        ...
        self.done_seed_find = False
        self.gamma = 0.9
        self.enemy_gamma = 0.6
        self.old_location = []
        self.first = first
    ...
    def setup_seeds(self):
        ...
        if self.local:
            for i in range(0,300):
                # print(f"seed is {i}")
                temp_env = gridworld.env(env_wrappers=[],render_mode=None)
                temp_env.reset(seed=i)
                map = temp_env.public_arena
                map = np.array(map).T  
                map = (map>>4)<<4
                self.possible_maps.append((map,i))
                # time.sleep(1)
                # print(map)
        else:
            for i in range(self.map_generator):
                i.reset()
                map = i.public_arena
                map = np.array(map).T  
                map = (map>>4)<<4
                self.possible_maps.append((map))
    def reset(self):
        self.reverse = False
    ...
    def check_maps(self):
        def convert_to_seed(x):
            return np.add(x==1,2*(x==5))
        # print(self.value_map)\\
        #HERE WE DO SEED HACKING
        seed_value_map = np.apply_along_axis(convert_to_seed, 0, self.value_map)
        # self.final_value=np.add(seed_value_map,self.final_value)
        self.combined_map #np.add(self.combined_map,seed_value_map)
        combined_map_bits =  np.unpackbits(self.combined_map, axis=-1)
    
        for i in range(len(self.possible_maps)-1,-1,-1):
            possible_map_bits = np.unpackbits(self.possible_maps[i][0], axis=-1)
            subset = np.all(possible_map_bits[combined_map_bits == 1] == 1)
            if not subset:
                # print("removing",i)
                # print(self.combined_map)
                # print(self.wall_map)
                # print(self.possible_maps[i][0])
                del self.possible_maps[i] 
                if not self.local:
                    del self.map_generator[i]
        if len(self.possible_maps) == 1 and not self.done_seed_find:
            # print("FOUND\n",self.possible_maps[0])
            # print(self.step,self.possible_maps[0][1])
            seed = self.possible_maps[0][1]
            if self.local :
                self.done_seed_find = True
                temp_env = gridworld.env(env_wrappers=[],render_mode=None)
                temp_env.reset(seed=seed)
                map = temp_env.public_state
                map = np.array(map).T  
                print(map)
                print(temp_env.agent_locations)
                if self.is_scout:
                    for location in temp_env.agent_locations.values():
                        if np.array_equal(location,np.zeros((2,))):
                            continue
                        self.add_last_known_position(location)
                    ...
                    for x, cell in enumerate(row):
                        for i in range(4):
                            ...
                            if self.is_scout:
                                ...
                                if self.value_map[y,x] == 2 and (cell>>(1))&1:
                                    self.value_map[y,x] = 1
                                elif self.value_map[y,x] == 2 and ((cell>>(0))&3) == 3:
                                    self.value_map[y,x] = 5
                            if (cell>>(4+i))&1:
                                # print("gay")
                                self.wall_map[y,x][i] = 1
            # raise Exception("Found")
        # if len(self.possible_maps)==0:
        #     input()
        #     raise Exception("Not possible")
        # print(self.combined_map)
        # print(len(self.possible_maps))
