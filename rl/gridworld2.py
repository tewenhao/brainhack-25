...
DEFAULT_REWARDS_DICT = {
    ...
    RewardNames.SCOUT_CAPTURED: -5,
    RewardNames.SCOUT_RECON: 1,
    RewardNames.SCOUT_MISSION: 2,
    RewardNames.STATIONARY_PENALTY:-.1,
    RewardNames.WALL_COLLISION:-.1,
    RewardNames.GUARD_TRUNCATION:-1,
    "scout_useless_movement":-.1,
    "GuardScaleReward":1,
    "StayingStillReward":-.1,
}
...
class raw_env(AECEnv[AgentID, ObsType, ActionType]):
    ...
    def __init__(
        ...
        self.debug = debug
        self.size: int = self.args.env_size  # size of the square grid
    ...
    def convert_array_to_bits(self,array,bits=8):
        array = np.array(array)
        return ((array[..., None] & (1 << np.arange(bits))) > 0).astype(np.uint8)
        ...
        return Dict(
            {
                ...
                "scout": Discrete(2),
                "location": Box(0, 16, shape=(2,), dtype=np.uint8),
    ...
    def observe(self, agent):
        ...
        # print("using dictionary return",view,self.agent_directions[agent],self.agent_locations[agent])
        if self.num_moves<0 or self.num_moves>NUM_ITERS:
            raise Exception(f"step out of limit currently {self.num_moves}")
        view = np.uint8(view)
        view = self.convert_array_to_bits(array = view)
        return {
            "viewcone": view,#np.uint8(view),
            "direction": np.uint8(self.agent_directions[agent]),
            "location": np.uint8(self.agent_locations[agent]),
            "scout": 1 if agent == self.scout else 0,
            "step": np.uint8(self.num_moves),
        }
    ...
    def reset(self, seed=None, options=None):
        ...
        self.num_moves = 0
        self.captures = {agent:0 for agent in self.agents}
        if self.render_mode in self.metadata["render_modes"]:
            self.render()
    ...
    def _move_agent(self, agent: AgentID, action: int):
        ...
        if _action in (Action.FORWARD, Action.BACKWARD):
            ...
            old_all_agent_location = copy.copy(self.agent_locations)
            if sum(direction) == 0:
            ...
            if agent == self.scout:
                ...
                if blank_tile:
                    self.rewards[self.scout] += self.rewards.get("scout_useless_movement",0)
            elif self.args.guard_distance_reward:
                scout_location = old_all_agent_location[self.scout]
                old_location = old_all_agent_location[agent]
                old_distance = abs(scout_location[0]-old_location[0])+abs(scout_location[1]-old_location[1])
                new_location = self.agent_locations[agent]
                new_distance = abs(scout_location[0]-new_location[0])+abs(scout_location[1]-new_location[1])
                self.rewards[agent] += int(old_distance-new_distance)
            return collision
