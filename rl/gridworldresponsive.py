...
class raw_env(AECEnv[AgentID, ObsType, ActionType]):
    ...
    def _move_agent(self, agent: AgentID, action: int):
        ...
        return None
    def _handle_individual_actions(self):
        """
        Handles any amount of actions at once, handling guard-scout captures, actions are executed in order and will raise error if out of order
        """
        while self.actions:
            current_agent = self.agent_selector.selected_agent         
            action = self.actions.get(current_agent,None)
            if action == None:
                raise Exception(f"Agent action requested not in the array of actions {self.actions}")
            capturers = []
            if current_agent == self.scout:
                # update scout location first
                def_collision = self._move_agent(self.scout, action)
                ...
                agent_collision = self._move_agent(current_agent, action)
            ...
            del self.actions[current_agent]
            # print(f"individual{self.rewards}")
    ...
    def step(self, action: ActionType):
        ...
        ) and self.agent_selection == self.agent_selector.is_first:
        ...
        # execute actions
        self._handle_individual_actions()
