...
from gridworldtypes import Action, Direction, Player, RewardNames, Tile, Wall

NUM_ITERS = 100
BIT_WEIGHT = 1 << np.arange(7, -1, -1)  # [128, 64, ..., 1]
...
class raw_env(AECEnv[AgentID, ObsType, ActionType]):
    ...
    def last_agent(
        self, agent, observe: bool = True
    ) -> tuple[ObsType | None, float, bool, bool, dict[str,str]]:
        ...
        if self.debug:
            ...
            for agent in self.agents:
                ...
                for x, y in np.ndindex((self.viewcone_length, self.viewcone_width)):
                    ...
                    if type(tile) != int:
                        tile = np.dot(tile, 1 << np.arange(0, 8, 1)).astype(np.uint8)
                        tile = int(tile) 
                    # draw whether the tile contains points
                    Tile(tile % 4).draw(
    ...
    def observe(self, agent):
        ...
        if True:#Split obs
            view = ((view[..., None] & (1 << np.arange(8))) > 0).astype(np.uint8)
        # print(view)
        return_obs = {
            "viewcone": view,
        ...
        if self.debug:
            return_obs["agent"] = agent
        return return_obs
    def observe_bypass(self, agent):
        """
        Returns the observation of the specified agent.
        """
        view = np.zeros((self.viewcone_length, self.viewcone_width), dtype=np.uint8)
    ...
    def _capture_scout_individual(self, capturers):
        """
        Given a list of agents who captured the scout, processes those agents' capture of the scout.
        Terminates the game and gives guards and the scout corresponding rewards.
        """
        ...
        self.terminations[self.scout] = True
        for agent in self.agents:
            if agent == self.scout:
                self.rewards[self.scout] += self.rewards_dict.get(
                    RewardNames.SCOUT_CAPTURED, 0
    ...
    def _move_agent(self, agent: AgentID, action: int):
        ...
        if _action in (Action.FORWARD, Action.BACKWARD):
            oldx,oldy = self.agent_locations[agent]
            scoutx,scouty =self.agent_locations[self.scout]
            old_distance = self.distance(oldx,oldy,scoutx,scouty)
            ...
            if agent == self.scout:
                ...
                tile = self._state[x, y]
                # print(f"added scout reward for {Tile.RECON}")
                # for other_agent in (self.agents):
                #     if other_agent == self.scout:
                #         continue
                #     guardx, guardy = self.agent_locations[other_agent]
                    
                    # if self.distance(x,y,guardx,guardy) <=1:
                    #     if guardx>x:
                    #         direction = 0
                    #     elif guardx<x:
                    #         direction = 2
                    ...
                    #     else:
                    #         direction =1
                    #     tile = self._state[tuple(self.agent_locations[self.scout])]
        # add 4 bc direction+4=wall bit
                        # if not get_bit(tile, direction + 4):
                        #     # self._handle_wall_collision(agent)
                        #     # return np.array([0, 0]), None
                        #     self.rewards[self.scout] += self.rewards_dict.get(RewardNames.SCOUT_CAPTURED,0)/5
                        #     self.rewards[other_agent] +=self.rewards_dict.get(RewardNames.GUARD_CAPTURES,0)/5
                match Tile(tile % 4):
                    case Tile.RECON:
                        self.rewards[self.scout] += self.rewards_dict.get(
                ...
                newx,newy = self.agent_locations[agent]
                scoutx,scouty =self.agent_locations[self.scout]
                new_distance = self.distance(newx,newy,scoutx,scouty)
                self.rewards[agent] += old_distance-new_distance
            return collision
    ...
    def _handle_actions_individual(self):
        """
        Handles all actions at once, handling guard-scout captures
        """
        capturers = []
        ...
        while self.actions:
            # update scout location first
            current_agent = self.agent_selector.selected_agent

            current_action = self.actions.pop(current_agent)
            # scout_action = self.actions.pop(self.scout)
            if current_agent == self.scout:
                # print("gay")

                def_collision = self._move_agent(current_agent, current_action)
                # if the scout walks into an guard, mark that guard as a capturer
                if def_collision is not None:
                    capturers.append(def_collision)
            else:
                # update agent location
                # pass
                agent_collision = self._move_agent(current_agent, current_action)
                # # if this guard walks into the scout, mark it as a capturer
                if agent_collision == self.scout:
                    
                    capturers.append(current_agent)

            # if there are any capturers, capture the scout
        if len(capturers) > 0:
            print("Capturers are",capturers)    
            self._capture_scout_individual(capturers)
    ...
    def step(self, action: ActionType):
        ...
        self._handle_actions_individual()
        ...
        if self.agent_selector.is_last():
            ...
            elif True in self.terminations.values():
